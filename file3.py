...
def testRun(name,bs,e,llr,ts,lstmnum):
    ...
    params = {
        "batch_size": bs,  # 20<16<10, 25 was a bust
        "epochs": e,
        "lr": llr,
        "time_steps": ts
    }
    ...
    OUTPUT_PATH = PATH_TO_DRIVE_ML_DATA + u"/outputs/lstm_best_14-5-19_9AM{0}/".format(name) + iter_changes
    ...
    def create_model():
        ...
        lstm_model.add(LSTM(lstmnum, batch_input_shape=(BATCH_SIZE, TIME_STEPS, x_t.shape[2]),
                            dropout=0.0, recurrent_dropout=0.0, #stateful=True, return_sequences=True,
                            kernel_initializer='random_uniform'))
        lstm_model.add(Dropout(0.4))
        '''
        lstm_model.add(LSTM(60, dropout=0.0, stateful=True, return_sequences=True))
        lstm_model.add(Dropout(0.4))
    ...
    while i > 0:
        i -= 1
        r += (y_pred_org[i] - y_test_t_org[i])**2

    print (name + ' error is equal to ' + str(r))
    return r


def predict(output_path,bs,ts,number):
    ...
    INPUT_PATH = PATH_TO_DRIVE_ML_DATA + "/inputs"
    OUTPUT_PATH = output_path + iter_changes
    TIME_STEPS = params["time_steps"]
    BATCH_SIZE = params["batch_size"]
    ...
    print(os.listdir(INPUT_PATH))
    # df_ge = pd.read_csv(os.path.join(INPUT_PATH, "ge.us.txt"), engine='python')
    df_ge = FPd.fetchData(number=number)
    print(df_ge.shape)
    print(df_ge.columns)
    # display(df_ge.head(5))
    # .pandas('Processing...')
    ...
    print(df_ge.dtypes)
    train_cols = ['bidopen', 'bidhigh', 'bidlow', 'bidclose', 'tickqty', "bid_hl", "bid_cl", "bid_ho", "bid_co"]
    df_train, df_test = train_test_split(df_ge, train_size=0.0, test_size=1.0, shuffle=False)
    print("Train--Test size", len(df_train), len(df_test))
    ...
    print("Are any NaNs present in train/test matrices?", np.isnan(x_test).any())

    saved_model = None
    try:
        saved_model = load_model(OUTPUT_PATH + '/best_model.h5')
        print("Loaded saved model...")
    except FileNotFoundError:
        print("Model not found")
