...
back = 15 # 40
fore = 4
x,y,t_x,t_y,v_x,v_y = getDataDFX('5e3data.csv',back,fore,1)
...
def mtrain(ep=250, cycles=25, bs=15, coshot=False):
    if coshot:
        CosHot = CosHotRestart(nb_epochs=ep, nb_cycles=cycles, gain=1.1, verbose=0)
        model.fit(t_x, t_y[:,1:], epochs=ep, verbose=2,batch_size=bs,  callbacks=[CosHot]) #)#
    else:
        SGDR = SGDRScheduler(min_lr=1e-6,max_lr=1e-2,
                             steps_per_epoch=np.ceil(55), #  len(t_x) / 23),
                             lr_decay=0.9,cycle_length=5,mult_factor=1.2)
        model.fit(t_x, t_y[:, 1:], epochs=ep, verbose=2, batch_size=bs, callbacks=[SGDR])
...
'''results(v_x,v_y, save=1)
x,y,t_x,t_y,v_x,v_y = getDataDFX('1e3data.csv',back,fore,1) # .012 maxval
results(v_x,v_y, save=1)
rtrain()
results(v_x,v_y, save=1)
x,y,t_x,t_y,v_x,v_y = getDataDFX('2e3data.csv',back,fore,1)
results(v_x,v_y, save=1)
rtrain()
results(v_x,v_y, save=1)
x,y,t_x,t_y,v_x,v_y = getDataDFX('5e3data.csv',back,fore,1) #
...
model = load_model('modMAX1559592272.h5')
#mtrain(bs=41,coshot=False)
model.fit(t_x, t_y[:, 1:], epochs=250, verbose=2, batch_size=41)
rtrain()
results(v_x,v_y, save=1)
# x,y,t_x,t_y,v_x,v_y = getDataDFX('5e3data.csv',back,fore,1) # .0208maxval
